...
void OCFFlapControl::init(){
    ...
    loaded = OCFFilesystem::readJsonFile(CAT_CONFIG_FILE, doc);
...
void OCFFlapControl::getStateJSON(String& outStr){
    DynamicJsonDocument doc(OCF_MAX_JSON_SIZE);
    doc["allow_out"] = config.allow_out;
    doc["allow_in"] = config.allow_in;
    doc["outside_state"] = stateMachineOutside->getState();
    doc["inside_state"] = stateMachineOutside->getState();
