...
void OCFStateMachine::update(){
    if (currentState == OCFMachineStates::IDLE) updateIdle();
    else if (currentState == OCFMachineStates::READING) updateReading();
    else if (currentState == OCFMachineStates::UNLOCKED) updateUnlocked();
}
...
void OCFStateMachine::updateReading(){
    ...
    // Return to Idle
    if (lastMotion <= millis() - 10000) transitionReadingToIdle();
    // Reset RFID after 5s
    ...
    // try to read tag id
    if (!rfidReading && serial->available() < 29) return; 
    ...
    serial->read();
    for (int x = 0; x < 29; x++) rfidTag[x] = 0x0;
    rfidReading = false;
    lastRFID = millis();
    digitalWrite(pin_rfid_reset, HIGH);
    digitalWrite(pin_rfid_reset, LOW);
